...
def find_best_threshold_by_f2(
    ...
    Mục tiêu:
    - Ưu tiên Recall/F2 cho class 1.
    - Không cho model predict toàn class 1.
    - Threshold chỉ được chọn nếu:
        + Positive_Rate <= max_positive_rate
        + Specificity >= min_specificity
        + Precision >= min_precision

    Nếu không có threshold nào thỏa ràng buộc:
    - Fallback sang threshold có Balanced Accuracy cao nhất.
    """
    ...
    if min_precision is None:
        min_precision = np.mean(y_true)

    for threshold in threshold_grid:

        y_pred = (y_prob >= threshold).astype(int)

        # Bỏ threshold nếu predict toàn 0 hoặc toàn 1
        if len(np.unique(y_pred)) < 2:
            continue
    ...
    if threshold_df.empty:
        print("\nWarning: No usable threshold candidates. Using threshold = 0.5")
        return 0.5, np.nan, threshold_df

    valid_df = threshold_df[threshold_df["Valid_Constraint"] == True]

    if len(valid_df) > 0:
        best_row = valid_df.sort_values(
            by=["F2_Score", "Recall", "Balanced_Accuracy", "Precision"],
            ascending=False
        ).iloc[0]

        print("\nThreshold selected by CONTROLLED F2.")
    else:
        print("\nWarning: No threshold satisfies controlled F2 constraints.")
        print("Fallback: selecting threshold by Balanced Accuracy.")

        best_row = threshold_df.sort_values(
            by=["Balanced_Accuracy", "F2_Score", "Recall", "Precision"],
            ascending=False
        ).iloc[0]
    ...
    print("\nSelected threshold diagnostics:")
    print(best_row)
