...
# Target: High Future Volatility Detection
# Threshold 1: High-volatility threshold tau_vol = Q75(Future_Volatility_train)
# Threshold 2: Classification threshold theta = maximize F2-score on Validation Set
# Metrics: Accuracy, Balanced Accuracy, Precision, Recall, F1, F2, ROC-AUC, MCC
# ==========================================
...
# Nếu chạy trong Colab/Jupyter và file nằm cùng thư mục với script, có thể sửa thành:
# DATA_PATH = "04_Master_Dataset.csv"

OUTPUT_DIR = "model_outputs_volatility"
os.makedirs(OUTPUT_DIR, exist_ok=True)

LOOKBACK = 14
VOLATILITY_QUANTILE = 0.75       # tau_vol = phân vị 75% trên tập train
RANDOM_STATE = 42
...
def find_best_threshold_by_f2(y_true, y_prob):
    """
    Chọn ngưỡng phân loại theta trên validation set bằng cách tối đa hóa F2-score.

    Lưu ý:
    - Đây là ngưỡng để chuyển xác suất mô hình thành nhãn dự báo 0/1.
    - Không phải ngưỡng tau_vol dùng để tạo Target.
    - F2-score ưu tiên Recall hơn Precision.
    - Phù hợp với mục tiêu cảnh báo biến động cao:
      bỏ lỡ ngày biến động mạnh được xem là rủi ro hơn so với cảnh báo giả.
    """
    ...
    for threshold in threshold_grid:
        ...
        # Tránh trường hợp mô hình dự báo toàn 0 hoặc toàn 1 trong quá trình chọn ngưỡng
...
if not os.path.exists(DATA_PATH):
    raise FileNotFoundError(
        ...
        "Hãy đặt file 04_Master_Dataset.csv vào thư mục data/ hoặc sửa DATA_PATH trong code."
    )
...
# Các biến này phản ánh bối cảnh thị trường và vĩ mô.
...
# ==========================================
# TARGET CONSTRUCTION: HIGH FUTURE VOLATILITY
# ==========================================

# Lợi suất logarit của Bitcoin
df["BTC_Return"] = np.log(df["Close"] / df["Close"].shift(1))

# Mức độ biến động tương lai = trị tuyệt đối của lợi suất ngày tiếp theo
df["Future_Volatility"] = df["BTC_Return"].shift(-1).abs()

required_cols = technical_features + fundamental_features + ["Future_Volatility"]

df = df.dropna(subset=required_cols)

print("\nDataset shape before Target:", df.shape)

# ==========================================
# TRAIN / VALIDATION / TEST SPLIT BEFORE TARGET THRESHOLD
# ==========================================
...
# Ngưỡng biến động cao tau_vol chỉ được tính trên tập train để tránh rò rỉ dữ liệu.
tau_vol = train_df["Future_Volatility"].quantile(VOLATILITY_QUANTILE)

print("\nHigh-volatility threshold tau_vol:", tau_vol)
print(f"tau_vol = Q{int(VOLATILITY_QUANTILE * 100)}%(Future_Volatility_train)")

for split_df in [train_df, val_df, test_df]:
    split_df["Target"] = (
        split_df["Future_Volatility"] > tau_vol
    ).astype(int)

# Ghép lại df nếu cần kiểm tra phân phối tổng thể sau khi gán Target
df = pd.concat([train_df, val_df, test_df], axis=0).sort_index()

print("\nFinal dataset shape:", df.shape)

print("\nTarget definition:")
print("Target = 1: Bitcoin có biến động giá cao trong ngày tiếp theo")
print("Target = 0: Bitcoin có biến động thấp/bình thường trong ngày tiếp theo")
...
hybrid_train_df["LSTM_Volatility_Probability"] = pure_lstm_train_prob
hybrid_val_df["LSTM_Volatility_Probability"] = pure_lstm_val_prob
hybrid_test_df["LSTM_Volatility_Probability"] = pure_lstm_test_prob

hybrid_features = [
    "LSTM_Volatility_Probability"
] + fundamental_features
...
hybrid_lstm_probability = pd.concat([
    pd.DataFrame({
        ...
        "Set": "Train",
        "LSTM_Volatility_Probability": pure_lstm_train_prob,
        "Actual": y_lstm_train
    }),
    pd.DataFrame({
        "Date": lstm_val_dates,
        "Set": "Validation",
        "LSTM_Volatility_Probability": pure_lstm_val_prob,
        "Actual": y_lstm_val
    }),
    pd.DataFrame({
        "Date": lstm_test_dates,
        "Set": "Test",
        "LSTM_Volatility_Probability": pure_lstm_test_prob,
        "Actual": y_lstm_test
    })
])

hybrid_lstm_probability.to_csv(
    os.path.join(OUTPUT_DIR, "hybrid_lstm_volatility_probability.csv"),
    index=False
)
...
target_info = pd.DataFrame({
    ...
    "Value": [
        VOLATILITY_QUANTILE,
        tau_vol,
        "Future_Volatility > tau_vol",
        "Future_Volatility <= tau_vol"
    ]
})

target_info.to_csv(
    os.path.join(OUTPUT_DIR, "target_volatility_definition.csv"),
    index=False
)
...
print("\nSaved files:")
print(f"- {OUTPUT_DIR}/model_comparison_metrics_train_val_test.csv")
print(f"- {OUTPUT_DIR}/all_model_predictions_train_val_test.csv")
print(f"- {OUTPUT_DIR}/test_model_comparison_f2.csv")
print(f"- {OUTPUT_DIR}/comparison_accuracy_table.csv")
print(f"- {OUTPUT_DIR}/comparison_balanced_accuracy_table.csv")
print(f"- {OUTPUT_DIR}/comparison_precision_table.csv")
print(f"- {OUTPUT_DIR}/comparison_recall_table.csv")
print(f"- {OUTPUT_DIR}/comparison_f1_table.csv")
print(f"- {OUTPUT_DIR}/comparison_f2_table.csv")
print(f"- {OUTPUT_DIR}/comparison_auc_table.csv")
print(f"- {OUTPUT_DIR}/comparison_mcc_table.csv")
print(f"- {OUTPUT_DIR}/pure_rf_feature_importance.csv")
print(f"- {OUTPUT_DIR}/hybrid_feature_importance.csv")
print(f"- {OUTPUT_DIR}/hybrid_lstm_volatility_probability.csv")
print(f"- {OUTPUT_DIR}/target_volatility_definition.csv")
